...
exports.getCoords = () => {//
	...
	console.log("WHAT IS FIN[1]: ", fin[1]);
	console.log("WHAT IS FIN[2]: ", fin[2]);

	let res = []; //result for position
	let satrec = satellite.twoline2satrec(fin[1], fin[2]); //Set satrec
	...
	console.log("Mins it takes to make 1 interval:", minsPerInterval);
	...
	let endTime = moment(julian.toDate(satrec.jdsatepoch).toISOString()).add(120, 'h').toISOString();; //add 25 hours to 
	//moment("2018-09-08T01:01:33.879Z").add(25, 'h').toISOString();
	...
	console.log("\n\nSetting intervals: \n\n");
	for (let i = 0; i <= 7200; i += minsPerInterval) {	//1500 == 25hours(which is our end time)
